...
export function RunDetailPage() {
  ...
  async function onPlanSkill() {
    setSkillPlanErr(null);
    if (events.length === 0) {
      setSkillPlanErr("当前没有观测事件，无法从 run 提炼 userGoal");
      return;
    }
    if (!hasIngestToken) {
    ...
    try {
      ...
      setSkillPlanErr(e instanceof Error ? e.message : "Plan 失败");
    } finally {
  ...
  return (
    <div className="ui-page">
      ...
      <section className="ui-section">
        <h3>生成 Skill（Plan → Generate）</h3>
        <p className="ui-meta" style={{ marginTop: 0 }}>
          第一步「Plan」读取当前 Run 观测并调用大模型提炼 userGoal（接口与模型输出均为纯文本，非 JSON）；第二步「Generate」仅根据 userGoal
          生成所选范式的 SKILL.md 等（服务端不再把 run 会话送入 Generate 模型）。zip 内按{" "}
          <code>cursor/&lt;skillId&gt;/</code> 与 <code>claude/&lt;skillId&gt;/</code>{" "}
          分目录存放。需 Bearer（VITE_AGENTIC_SERVER_TOKEN 或 AGENTIC_SERVER_TOKEN）。可选临时 LLM apiKey。
        </p>
        ...
        <label style={{ display: "block", marginBottom: 10 }}>
          ...
          <input
            ...
            placeholder="覆盖服务端 AGENTIC_SKILL_LLM_API_KEY，仅本次请求"
          />
        </label>
        <button
          className="ui-btn"
        ...
        >
          {skillPlanBusy ? "提炼中…" : "从 Run 提炼 userGoal（Plan）"}
        </button>
        <button
          className="ui-btn"
          type="button"
          ...
          .map((e) => (
            <li key={`${e.agentId}-${e.seq}-${e.id}`}>
              <div className="ui-meta">
                seq {e.seq} · {e.kind} · {e.provider} · {e.agentId} · {e.ts}
              </div>
