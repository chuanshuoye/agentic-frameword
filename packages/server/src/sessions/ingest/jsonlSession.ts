...
import { upsertProject, upsertSessions, type SessionSourceType, type SessionUpsertInput } from "../../store.js";

export type JsonlIngestContext = {
...
export function syncJsonlTranscriptsDir(
  db: Database.Database,
  transcriptsDir: string,
  ctx: JsonlIngestContext,
  ...
  for (const filePath of files) {
    const result = parseTranscriptFile(root, filePath, ctx);
    if (result) {
      parsed.push(result);
    } else {
      skipped += 1;
...
function parseTranscriptFile(
  ...
  const projectPath = pickProjectPath(first, last, relativePath, ctx.sourceType);
  const projectName = basename(projectPath);
  const projectKey = toProjectKey(projectName, projectPath);
  const sourceAgentId = pickAgentId(first, last, relativePath, ctx.defaultAgentId);
...
function pickProjectPath(
  ...
  if (sourceType === "claude_local" && parts.length === 1) {
    return parts[0];
  }
